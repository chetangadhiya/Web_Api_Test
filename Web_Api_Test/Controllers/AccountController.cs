...
namespace Web_Api_Test.Controllers
{
    ...
    public class AccountController : ApiController
    {
        ...
        [HttpPost]
        // GET: Account
        public IHttpActionResult Login(SignUp signUp)
        {
            var userName = signUp.UserName;
            var password = signUp.Password;

            var UserData = db.SignUps.Where(x => x.UserName.Equals(userName) && x.Password.Equals(password)).FirstOrDefault();
            

            if (UserData != null)
            {
                var getToken = GetToken(UserData);
                return Ok(getToken);
            }
            return Unauthorized();
        ...
        public object GetToken(SignUp UserData)
        {
